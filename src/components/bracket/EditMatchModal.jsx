...
export default function EditMatchModal({ match, onClose, onSave }) {
  ...
  return (
    <Dialog
      ...
      onClose={onClose}
      labelledBy={titleId}
      backdropClassName="fixed inset-0 z-[120] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4"
      panelClassName="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-in zoom-in-95 duration-200 max-h-[92vh] overflow-y-auto"
    >
      <h3 id={titleId} className="text-xl font-bold mb-4 text-slate-800">
        Editar Partido
      </h3>

      <div className="space-y-4">
        <div>
          <label htmlFor={titleFieldId} className="block text-sm font-semibold text-slate-700 mb-1">
            Título de la Ronda
          </label>
          <input
            id={titleFieldId}
        ...
        <div>
          <label htmlFor={formatId} className="block text-sm font-semibold text-slate-700 mb-1">
            Formato
          </label>
          <select
            id={formatId}
        ...
        <fieldset className="pt-1">
          <legend className="block text-sm font-semibold text-slate-700 mb-2">Horarios y lugar</legend>
          <div className="space-y-3">
            {Array.from({ length: size }).map((_, i) => (
              <div
                ...
                role="group"
                aria-label={size > 1 ? `Partido ${i + 1}` : 'Partido'}
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <input
                    type="date"
                    aria-label={`Fecha ${size > 1 ? `partido ${i + 1}` : ''}`}
                    value={dates[i] || ''}
                  ...
                  <input
                    type="time"
                    aria-label={`Hora de inicio ${size > 1 ? `partido ${i + 1}` : ''}`}
                    value={times[i] || ''}
                  ...
                  <input
                    type="time"
                    aria-label={`Hora de fin ${size > 1 ? `partido ${i + 1}` : ''}`}
                    value={endTimes[i] || ''}
                ...
                <input
                  type="text"
                  aria-label={`Lugar ${size > 1 ? `partido ${i + 1}` : ''}`}
                  value={places[i] || ''}
      ...
      <div className="flex justify-end gap-3 mt-6">
        <button
          type="button"
          onClick={onClose}
          className="px-4 py-2 text-slate-600 font-semibold bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2"
        >
          Cancelar
        </button>
        <button
          type="button"
          onClick={handleSave}
          className="px-4 py-2 bg-blue-600 text-white font-bold rounded-lg shadow-md hover:bg-blue-700 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2"
        >
          Guardar
        </button>
      </div>
