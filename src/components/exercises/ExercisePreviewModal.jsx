...
export default function ExercisePreviewModal({ exercise, sharing, onClose, onEdit, onCreateVariant, onShare }) {
  ...
  return (
    <Dialog
      ...
      panelClassName="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200"
    >
      <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-slate-100">
        <div className="min-w-0 flex-1">
          <h2 id={titleId} className="text-xl font-bold text-slate-800 truncate">
            {exercise.nombre}
          </h2>
        ...
        <button
          ...
          onClick={onClose}
          aria-label="Cerrar"
          className="text-slate-400 hover:text-slate-600 ml-4 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2 rounded"
        >
      ...
      <div className="overflow-y-auto flex-1 p-6">
        <div ref={contentRef} className="bg-white p-2">
          <h3 className="text-lg font-bold text-slate-800 mb-3">{exercise.nombre}</h3>
      ...
      <div className="px-6 py-4 border-t border-slate-100 space-y-3">
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2"
            title="Descargar como imagen"
          >
            <Image size={14} aria-hidden="true" /> PNG
          </button>
          <button
            type="button"
            onClick={() => onShare(exercise)}
            disabled={sharing}
            className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2"
            title="Compartir por enlace"
          >
            <Share2 size={14} aria-hidden="true" /> {sharing ? 'Compartiendo...' : 'Compartir'}
          </button>
        </div>
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2"
          >
