...
export default function DashboardScreen() {
  ...
  const [linkingBracketId, setLinkingBracketId] = useState(null);
  return (
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-end sm:items-center justify-center p-4 backdrop-blur-sm transition-opacity">
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 animate-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-600 mb-6 text-sm">
              Esta acción borrará el torneo y todos sus resultados permanentemente.
            </p>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {firebaseError && (
          <div className="bg-amber-50 border border-amber-200 text-amber-800 text-sm p-4 rounded-xl mb-6 flex items-center gap-3">
            ⚠️ Estás usando la aplicación sin configurar la Autenticación de Firebase localmente. Los torneos se
            borrarán si recargas la página.
          </div>
        )}
        ...
        {brackets.length > 0 && (
          <div className="flex flex-col sm:flex-row gap-3 mb-6">
            ...
            <select
              ...
              className="px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm bg-white"
            >
            ...
            return (
              <div className="bg-white border-2 border-dashed border-slate-300 rounded-2xl p-10 text-center shadow-sm">
                <p className="text-slate-500">
                  No se encontraron torneos para "<strong>{dashboardSearch}</strong>"
                </p>
              </div>
            );
                          ...
                          <div className="absolute left-0 top-full mt-1 z-20 bg-white border border-slate-200 rounded-lg shadow-xl py-1 min-w-[200px] max-h-48 overflow-y-auto">
                            {coachTeams.map((t) => (
                              <button
                                key={t.id}
                                onClick={() => {
                                  handleLinkTeam(b.id, t);
                                  setLinkingBracketId(null);
                                }}
                  ...
                  <div className="flex justify-between items-center mt-auto border-t border-slate-100 pt-4">
                    <button
                      onClick={() => {
                        ...
                        localStorage.setItem('playoffs:lastActiveBracketId', b.id);
                        setAppMode('bracket');
                        setZoom(1);
                      }}
