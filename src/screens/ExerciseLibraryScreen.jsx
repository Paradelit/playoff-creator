...
import {
  ArrowLeft, Plus, Pencil, Trash2, X, BookOpen, FolderOpen,
  Undo, Maximize2, Download, Upload, CheckSquare, Square,
} from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { useFirebase } from '../contexts/FirebaseContext';
import { subscribeToExercises, saveExercise, deleteExercise, propagateExerciseUpdate } from '../services/trainingsService';
...
export default function ExerciseLibraryScreen() {
  ...
  function updateElementos(nuevos) {
    setEditingExercise(ex => ({ ...ex, elementos: typeof nuevos === 'function' ? nuevos(ex.elementos || []) : nuevos }));
  }
  ...
  function doExport() {
    const selected = exercises.filter(ex => exportSelected.has(ex.id)).map(({ id, createdAt, updatedAt, ...rest }) => rest);
    const blob = new Blob([JSON.stringify({ version: 1, exportDate: new Date().toISOString(), exercises: selected }, null, 2)], { type: 'application/json' });
  ...
  function handleImportFile(e) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (!list) { alert('Archivo no válido.'); return; }
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-6 sm:p-12 font-sans pb-24">
      <div className="max-w-4xl mx-auto">
        ...
        <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-slate-500 hover:text-slate-700 text-sm font-medium transition mb-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Nuevo */}
            <button
              onClick={() => { setEditingExercise({ ...EMPTY_EXERCISE }); setShowPlaybook(false); setActiveTool('O'); }}
        ...
        {loading ? (
          ...
          <div className="bg-white border-2 border-dashed border-slate-300 rounded-2xl p-16 text-center shadow-sm">
            ...
            <button onClick={() => { setEditingExercise({ ...EMPTY_EXERCISE }); setShowPlaybook(false); setActiveTool('O'); }} className="text-blue-600 font-bold hover:underline text-sm">
              Crear ejercicio
            </button>
          </div>
        ) : filtered.length === 0 ? (
              ...
              <div key={ex.id} className="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-end gap-2 mt-3">
                    <button onClick={() => { setEditingExercise({ ...ex, elementos: ex.elementos || [] }); setShowPlaybook(false); setActiveTool('O'); }} className="text-slate-400 hover:text-blue-600 p-2 hover:bg-blue-50 rounded-lg transition-colors" title="Editar">
                      <Pencil size={15} />
                    </button>
                    <button onClick={() => setDeletingId(ex.id)} className="text-red-400 hover:text-red-600 p-2 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar">
                      <Trash2 size={15} />
                    </button>
                  </div>
                </div>
      ...
      {/* ─── Modal exportar ─── */}
      {showExport && (
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-end sm:items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowExport(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[80vh] flex flex-col animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-slate-100">
              <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <Download size={18} className="text-blue-600" /> Exportar ejercicios
              </h3>
              <button onClick={() => setShowExport(false)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
            </div>

            {/* Seleccionar todos */}
            <div className="px-6 py-3 border-b border-slate-100">
              <button onClick={toggleExportAll} className="flex items-center gap-2 text-sm font-semibold text-slate-700 hover:text-blue-600 transition">
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
              <button onClick={() => setShowExport(false)} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3 rounded-xl transition">
                Cancelar
              </button>
              <button
                onClick={doExport}
      ...
      {importPreview && (
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-end sm:items-center justify-center p-4 backdrop-blur-sm" onClick={() => setImportPreview(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[80vh] flex flex-col animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-slate-100">
              <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <Upload size={18} className="text-blue-600" /> Importar ejercicios
              </h3>
              <button onClick={() => setImportPreview(null)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
            </div>

            <p className="px-6 py-3 text-sm text-slate-500 border-b border-slate-100">
              Se añadirán <span className="font-bold text-slate-700">{importPreview.length} ejercicio{importPreview.length !== 1 ? 's' : ''}</span> a tu biblioteca. Los ejercicios existentes no se modificarán.
            </p>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
              <button onClick={() => setImportPreview(null)} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3 rounded-xl transition">
                Cancelar
              </button>
              <button
                onClick={doImport}
            ...
            <form onSubmit={handleSave} className="px-6 py-5 flex flex-col gap-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={() => setShowPlaybook(true)} className="flex items-center gap-1.5 text-xs font-bold text-blue-600 hover:text-blue-800 transition">
                    <Maximize2 size={13} /> Abrir editor
                  </button>
                </div>
                <div className="h-32 rounded-xl border border-slate-200 bg-gray-50 flex items-center justify-center cursor-pointer hover:bg-gray-100 transition overflow-hidden" onClick={() => setShowPlaybook(true)}>
                  <CourtCanvas tipo={editingExercise.tipoPista} elementos={editingExercise.elementos || []} readOnly={true} />
                </div>
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setEditingExercise(null)} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3 rounded-xl transition-colors">Cancelar</button>
                <button type="submit" disabled={saving} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-colors disabled:opacity-60">
          ...
          <div className="bg-white w-full max-w-5xl h-[85vh] rounded-lg shadow-2xl flex flex-col overflow-hidden">
            <div className="flex justify-between items-center p-3 border-b border-gray-200 bg-gray-50">
              <div className="flex items-center gap-4">
                ...
                <div className="flex gap-1 border-l border-gray-300 pl-4">
                  <button onClick={() => updateElementos(els => els.slice(0, -1))} className="p-1.5 text-gray-600 hover:bg-gray-200 rounded flex items-center text-sm">
                    <Undo size={14} className="mr-1" /> Deshacer
                  </button>
                  <button onClick={() => updateElementos([])} className="p-1.5 text-red-600 hover:bg-red-50 rounded flex items-center text-sm">
                    <Trash2 size={14} className="mr-1" /> Limpiar
                  </button>
                  <div className="flex gap-1 border-l border-gray-300 pl-3 ml-1">
                    {[['media', 'Media pista'], ['entera', 'Pista entera']].map(([val, label]) => (
                      <button key={val} onClick={() => setEditingExercise(ex => ({ ...ex, tipoPista: val }))}
                        className={`px-3 py-1 rounded text-sm font-semibold transition-colors ${editingExercise.tipoPista === val ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                        {label}
                      </button>
                    ))}
                  </div>
              ...
              <button onClick={() => setShowPlaybook(false)} className="p-1.5 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 transition">
                <X size={18} />
              </button>
            </div>
            <div className="flex flex-1 overflow-hidden bg-gray-100">
              <div className="w-48 bg-white border-r border-gray-200 flex flex-col p-2 gap-1 overflow-y-auto">
                ...
                {COURT_TOOLS.map((t, idx) => t.divider ? (
              ...
              <div className="flex-1 flex items-center justify-center p-6 select-none">
                <div className="bg-white shadow border border-gray-300 w-full h-full flex items-center justify-center">
                  <CourtCanvas tipo={editingExercise.tipoPista} elementos={editingExercise.elementos || []} setElementos={updateElementos} readOnly={false} activeTool={activeTool} />
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setDeletingId(null)}>
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={() => handleDelete(deletingId)} className="px-4 py-2 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition shadow-sm">Sí, eliminar</button>
            </div>
          </div>
        </div>
      )}
