...
export default function UploadScreen({
  ...
  basesFile, setBasesFile,
  clasifFile, setClasifFile,
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
      <div className="max-w-2xl w-full bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-200">
        ...
        <div className="p-8">
          ...
          {isProcessing
            ? <div className="mt-6 flex flex-col items-center p-4 bg-indigo-50 rounded-xl"><Loader2 size={32} className="text-indigo-600 animate-spin mb-3" /><span className="text-indigo-800 text-sm font-medium">{processStatus}</span></div>
            : <button onClick={handleProcessDocuments} disabled={!basesFile || !clasifFile || !newBracketName.trim()} className="mt-6 w-full flex items-center justify-center gap-2 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-md transition-all disabled:opacity-50 disabled:cursor-not-allowed">Analizar y Construir Cuadro <ArrowRight size={20} /></button>
          }
        </div>
      </div>
    </div>
